"""Unit tests for CopilotProvider implementation.

Tests cover the Popen-based Copilot provider for Multi-LLM validation.

Tests cover:
- AC1: CopilotProvider extends BaseProvider
- AC2: provider_name returns "copilot"
- AC3: default_model returns "gpt-4o"
...
from bmad_assist.providers.copilot import (
    DEFAULT_TIMEOUT,
...
class TestCopilotProviderStructure:
    """Test AC1, AC2, AC3: CopilotProvider class definition."""

    def test_provider_inherits_from_baseprovider(self) -> None:
        """Test AC1: CopilotProvider inherits from BaseProvider."""
        assert issubclass(CopilotProvider, BaseProvider)

    def test_provider_has_class_docstring(self) -> None:
        """Test AC1: CopilotProvider has docstring explaining its purpose."""
        assert CopilotProvider.__doc__ is not None
        assert "copilot" in CopilotProvider.__doc__.lower()

    def test_provider_name_returns_copilot(self) -> None:
        """Test AC2: provider_name returns 'copilot'."""
    ...
    def test_default_model_returns_gpt_4o(self) -> None:
        """Test AC3: default_model returns 'gpt-4o'."""
        provider = CopilotProvider()
...
class TestCopilotProviderModels:
    ...
    @pytest.fixture
    def provider(self) -> CopilotProvider:
        """Create CopilotProvider instance."""
        return CopilotProvider()

    def test_supports_model_always_returns_true(self, provider: CopilotProvider) -> None:
        ...
        assert provider.supports_model("claude-sonnet") is True
...
class TestCopilotProviderInvoke:
    ...
    def test_invoke_builds_correct_command(
        self, provider: CopilotProvider, mock_popen_success: MagicMock
    ) -> None:
        """Test AC5: invoke() builds command with correct flags."""
        provider.invoke("Review code", model="gpt-4o")
    ...
    def test_invoke_returns_providerresult_on_success(
        ...
        result = provider.invoke("Hello", model="gpt-4o", timeout=30)
    ...
    def test_invoke_providerresult_has_model(
        ...
        result = provider.invoke("Hello", model="gpt-4-turbo")
...
class TestCopilotProviderErrors:
    ...
    def test_invoke_raises_timeout_error(self, provider: CopilotProvider) -> None:
    ...
    def test_invoke_raises_exit_code_error(self, provider: CopilotProvider) -> None:
    ...
    def test_invoke_raises_provider_error_when_cli_not_found(
        ...
        with patch("bmad_assist.providers.copilot.Popen") as mock_popen:
            mock_popen.side_effect = FileNotFoundError("copilot")
...
class TestCopilotProviderSettings:
    ...
    def test_invoke_accepts_settings_file(
        self, provider: CopilotProvider, mock_popen_success: MagicMock, tmp_path: Path
    ) -> None:
...
class TestCopilotProviderUnicode:
    """Test Unicode handling in CopilotProvider."""
...
class TestParseOutput:
    ...
    def test_parse_output_strips_whitespace(self, provider: CopilotProvider) -> None:
    ...
    def test_parse_output_empty_string(self, provider: CopilotProvider) -> None:
...
class TestDocstringsExist:
    ...
    def test_module_has_docstring(self) -> None:
        ...
        from bmad_assist.providers import copilot

        assert copilot.__doc__ is not None
        assert "copilot" in copilot.__doc__.lower()

    def test_provider_has_docstring(self) -> None:
        """Test CopilotProvider has docstring."""
        assert CopilotProvider.__doc__ is not None

    def test_invoke_has_docstring(self) -> None:
        """Test invoke() has docstring."""
        assert CopilotProvider.invoke.__doc__ is not None

    def test_parse_output_has_docstring(self) -> None:
        """Test parse_output() has docstring."""
        assert CopilotProvider.parse_output.__doc__ is not None
