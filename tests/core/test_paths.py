...
class TestProjectPaths:
    ...
    def test_default_project_knowledge_is_docs(self, project_root: Path):
        """Default project knowledge is docs/, not planning_artifacts."""
        project_root.mkdir(parents=True)
        (project_root / "_bmad-output" / "planning-artifacts").mkdir(parents=True)
        paths = ProjectPaths(project_root)
        # docs/ is the canonical location for project documentation,
        # planning_artifacts is an output dir searched separately
        assert paths.project_knowledge == project_root.resolve() / "docs"

    def test_default_project_knowledge_without_planning(self, project_root: Path):
        """Default project knowledge is docs/ even without planning_artifacts."""
        project_root.mkdir(parents=True)
        paths = ProjectPaths(project_root)
        assert paths.project_knowledge == project_root.resolve() / "docs"
...
class TestProjectKnowledgePaths:
    ...
    def test_prd_file(self, project_root: Path):
        """PRD file path derives from project_knowledge (docs/)."""
        project_root.mkdir(parents=True)
        paths = ProjectPaths(project_root)
    ...
    def test_architecture_file(self, project_root: Path):
        """Architecture file path derives from project_knowledge (docs/)."""
        project_root.mkdir(parents=True)
        paths = ProjectPaths(project_root)
    ...
    def test_project_context_file(self, project_root: Path):
        """Project context file path derives from project_knowledge (docs/)."""
        project_root.mkdir(parents=True)
        paths = ProjectPaths(project_root)
